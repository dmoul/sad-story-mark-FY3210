...
In 1933, in the depths of the Depression, the U.S. Coast & Geodetic Survey (precursor to the NGS), placed mark [FY3210](https://www.ngs.noaa.gov/cgi-bin/ds_mark.prl?PidBox=FY3210) near the top of Occoneechee Mountain. How do we know? Because each mark has its own data sheet, and the datasheet tells a story.
...
NAD 83(2001) is the North American Datum^[See @sec-terminology ] of 1983, the standard used by the NGS today. The horizontal position is provided in degrees, minutes, seconds in the North and West directions (West of the [prime meridian](https://en.wikipedia.org/wiki/Prime_meridian_(Greenwich)) at Greenwich, England). You can find Occoneechee Mountain on [OpenStreetMap](https://www.openstreetmap.org/relation/9742275) by searching for *Occoneechee Mountain* or latitude and longitude: *36°03'42.6"N 79°07'18.6"W* (or the decimal equivalent: *36.06182, -79.12182*).

NAVD 88 is the North American Vertical Datum of 1988, also the current standard. The Orthographic height ("ortho height") is defined relative to a datum. This mark is 262 meters (860 ft) above sea level.

## Four counties in NC

I will be showing maps that include Orange, Durham, Wake, and Chatham Counties. They are in the North Carolina [Piedmont](https://en.wikipedia.org/wiki/Central_North_Carolina) and above the [fall line](https://en.wikipedia.org/wiki/Fall_line).^[Map of North Carolina with fall line <https://www.climate.gov/media/12938>  Image caption: "The fall line in North Carolina separates the Piedmont from the Coastal Plain. East of the line, there is little high ground, and many cities and towns sit in or next to the floodplains of the region's many rivers. NOAA Climate.gov image, adapted from digital elevation model data from [NOAA NCEI's bathymetric data viewer](https://maps.ngdc.noaa.gov/viewers/bathymetry/)."] For more about these counties and North Carolina in general, see [North Carolina trends: geography, population, income, and education](https://dmoul.github.io/nc-trends/), which I completed in April 2023.
...
Occoneechee Mountain is the highest elevation in the four counties where most of the people in "[the triangle](https://en.wikipedia.org/wiki/Research_Triangle)" live. @fig-counties-highest-lowest-elevations presents some of the highest and lowest points of elevation in these counties. Where multiple points are close together I show only one.^[Note that the mark PID [EZ2890](https://www.ngs.noaa.gov/cgi-bin/ds_mark.prl?PidBox=EZ2890) near White Cross must be wrong. 
...
At the location of PID EZ2890 the elevation reported by SRTM is a more reasonable 231 m (757.9 ft), which if accurate, 
indicates the orhto_ht of [EZ2890](https://www.ngs.noaa.gov/cgi-bin/ds_mark.prl?PidBox=EZ2890) has a 60% error. 
So I use the STRM elevation instead in @fig-counties-highest-lowest-elevations.]

```{r fig.height=8, fig.width=12}
#| label: fig-counties-highest-lowest-elevations
#| fig-cap: "Some of the survey marks with highest and lowest elevations"
#| fig-height: 8
#| fig-width: 12
#| column: screen-right
#| warning: false
...
The next portion is of interest to surveyors more than the general public. Surveyors go to school and pass a professional exam for a reason, and I'm content to trust them in their pursuit of accurate measurements. I'll simply make four points:

1. The geoid is the imaginary sphere around the earth at sea level.^[See @sec-terminology ] The datum is based on the geoid.
1. The 'GEOID18 height accuracy estimate' page referenced below (<https://www.ngs.noaa.gov/GEOID/GEOID18/computation.html>) reports a vertical accuracy of 0.030 m (3 mm or 1.2 inches) with 95% confidence interval (orthographic height has it's own additional error).
...
1. The reference marks provide a starting point to recalculate a mark's location.
